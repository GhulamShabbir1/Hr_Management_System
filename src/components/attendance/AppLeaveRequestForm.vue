    ...
    <div class="card p-4 mb-4 gradient-blue text-white">
      ...
      <form @submit.prevent="submitRequest">
        <div class="row g-3">
          ...
          <div class="col-12">
            <button class="btn btn-light" type="submit" :disabled="submitting">
            ...
            <table class="table table-hover align-middle">
              ...
              <tbody>
                ...
                <tr v-if="myRequests.length === 0">
                  <td colspan="6" class="text-center text-muted py-3">No requests</td>
                </tr>
              </tbody>
            </table>
          </div>
              ...
              <tbody>
                <tr v-for="(r, idx) in pendingRequests" :key="r.id || idx">
                  ...
                  <td class="d-flex gap-2">
                    <button class="btn btn-sm btn-success" @click="updateStatus(r, 'approved')">Approve</button>
                    <button class="btn btn-sm btn-danger" @click="updateStatus(r, 'rejected')">Reject</button>
                  </td>
                </tr>
...
export default {
  ...
  // Make userRole optional; fallback to store to avoid prop warnings
  props: {
  ...
  computed: {
    ...
    isApprover() {
      const role = this.userRole || this.$store.getters['auth/userRole'];
  ...
  methods: {
    ...
    async loadEmployees() {
      try {
        ...
        this.employees = (Array.isArray(payload) ? payload : [])
          .map(this.normalizeEmployee).filter(e => e.id != null);
      } catch {
        this.employees = [];
      }
    },
    async loadMyRequests() {
      try {
        ...
        const payload = res?.data?.data ?? res?.data?.items ?? res?.data ?? [];
        this.myRequests = (Array.isArray(payload) ? payload : []).map(this.normalizeRequest);
      } catch {
        this.myRequests = [];
      }
    },
    async loadPending() {
      ...
      try {
        ...
        const payload = res?.data?.data ?? res?.data?.items ?? res?.data ?? [];
        this.pendingRequests = (Array.isArray(payload) ? payload : []).map(this.normalizeRequest);
      } catch {
    ...
    async submitRequest() {
      try {
        ...
        // Send all aliases to satisfy backend validation (prevents 422)
        ...
        const body = {
          // As per your endpoint list
          start_date: start,
          end_date: end,
          type,
          reason,
          // Common aliases some backends require
          from: start,
          to: end,
          leave_type: type,
          message: reason
        ...
        await this.$axios.post('/leave/request', body);
        ...
        this.toast(this.extractErrorMessage(e), 'error');
      } finally {
...
.badge.bg-success, .badge.bg-danger, .badge.bg-warning { font-size: 0.85rem; }
</style>