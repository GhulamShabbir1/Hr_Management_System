          ...
          <table class="table table-hover align-middle table-modern">
            ...
            <tbody>
              <tr v-for="(task, index) in filteredTasks" :key="task.id || (task.title + '-' + index)">
                ...
                <td><span class="badge badge-pill" :class="getStatusBadgeClass(task.status)">{{ task.status }}</span>
                </td>
                <td><span class="badge badge-pill" :class="getPriorityBadgeClass(task.priority)">{{ task.priority
                    }}</span></td>
                ...
                <td>
                  <div class="btn-group btn-group-sm">
                    <button @click="toggleStatus(task)" class="btn btn-outline-primary" title="Toggle status">
                      <i class="bi"
                        :class="task.status === 'Completed' ? 'bi-arrow-counterclockwise' : 'bi-check2-circle'"></i>
                    </button>
      ...
      <div class="card card-elevated p-3">
        ...
        <div class="row g-3">
          ...
          <div class="col-12">
            ...
            <textarea v-model="evaluation.feedback" class="form-control" rows="3"
              placeholder="Feedback about the task"></textarea>
          </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mt-3">
              ...
              <textarea v-model="newTask.description" rows="3" class="form-control"
                placeholder="Task description"></textarea>
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
            <button class="btn btn-gradient-primary" @click="confirmSaveTask"
  ...
  methods: {
    ...
    normalizeTask(t) {
      ...
      return {
        ...
        employeeId: t.employeeId ?? t.user_id ?? t.userId ?? '',
    ...
    async toggleStatus(task) {
      ...
      try {
        // Try a common update route; if not supported, keep UI change
    ...
    async confirmSaveTask() {
      try {
        if (this.editMode && this.editingTask?.id != null) {
          // Attempt update; fallback to local change
          try {
            await this.$axios.put(`/update-task/${this.editingTask.id}`, this.newTask);
            await this.loadTasks();
            this.notify({ type: 'success', title: 'Success', text: 'Task updated' });
          } catch {
        ...
        this.notify({ type: 'error', title: 'Error', text: this.editMode ? 'Failed to update task' : 'Failed to assign task' });
      }
    },
    async removeTask(id) {
      if (!id) return;
      ...
      try {
        // No delete endpoint in provided API; remove locally
